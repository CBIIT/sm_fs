...
export const DEBUG_ERROR_INTERCEPTOR = new InjectionToken<boolean>('debugMode', {providedIn: 'root', factory: () => false});
    ...
    return next.handle(req).pipe(
      catchError((error) => {
        ...
        if (error.status === 200 && error.url?.startsWith('https://auth')) {
          if(this.debugMode) this.logger.debug('Timeout encountered - redirect to login.', error);
          this.heartbeatService.pause();
